...
한국 KOSPI + KOSDAQ 상장 기업 중 **영업이익률 ≥ 20% AND ROE ≥ 20%** 인 기업을 DART API로 자동 선별하고, 분기마다 Gmail로 HTML 보고서를 발송하는 Python 툴.
...
├── run.bat               # Task Scheduler용 배치 파일
├── scheduler_setup.ps1   # Windows 작업 스케줄러 등록 스크립트
...
**미국 섹션** (레드 헤더): 순위 / 티커 / 종목명 / 섹터 / 영업이익률 / ROE (TTM 기준)
...
`scheduler_setup.ps1` 실행 시 Windows 작업 스케줄러에 4개 작업 등록:
- 매년 3/15, 6/15, 9/15, 12/15 오전 09:00
- 실패 시 30분 후 최대 2회 재시도
